...
pcall(function()
	local url = "https://raw.githubusercontent.com/afraidly/id-tracker/refs/heads/main/espstuff.lua"
...
local function EvaluateParryTriggers()
	...
	for _, character in ipairs(TargetCharacters) do
		...
		if dist <= AutoParryRange and EspTrackers[character] and EspTrackers[character].ChangeText then
			EspTrackers[character]:ChangeText("Name", character.Name .. " IN RANGE", COLOR_GREEN)
		elseif EspTrackers[character] and EspTrackers[character].ChangeText then
...
local function UpdateTargetCharacters(charactersList)
	...
	for _, character in ipairs(charactersList) do
		...
		if character and character:FindFirstChild("HumanoidRootPart") and ESP_Utility then
			pcall(function()
